...
import { PlusCircle, Edit2, Save, X, Image as ImageIcon, Upload, Trash2 } from 'lucide-react';
                ...
                <div className="space-y-6">
                  <div className="grid grid-cols-2 gap-4">
                    <div className="space-y-1">
                      ...
                      <input type="text" value={formData.sku} onChange={e => setFormData({...formData, sku: e.target.value})} className="w-full px-4 py-3 rounded-2xl border border-slate-200 font-bold text-sm outline-none focus:border-blue-500 transition-all bg-slate-50" placeholder="AUTOGEN" />
                    </div>
                    <div className="space-y-1">
                  ...
                  <div className="grid grid-cols-3 gap-4">
                    ...
                    <div className="space-y-1">
                      <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest ml-1">U. Medida</label>
                ...
                <div className="space-y-8">
                      ...
                      <div className="w-48 h-48 bg-white rounded-4xl border-2 border-slate-100 overflow-hidden shadow-inner flex items-center justify-center relative group">
                        {formData.image ? (
                          <>
                            ...
                            <button type="button" onClick={() => setFormData({...formData, image: ''})} className="absolute inset-0 bg-red-600/80 text-white opacity-0 group-hover:opacity-100 transition-opacity flex items-center justify-center font-black gap-2">
                  ...
                  <div className="grid grid-cols-2 gap-6 bg-blue-50/50 p-6 rounded-[2.5rem] border border-blue-100">
                    <div className="space-y-1">
                      ...
                      <input type="number" step="0.01" required value={formData.price} onChange={e => setFormData({...formData, price: e.target.value})} className="w-full px-5 py-4 rounded-2xl border-2 border-blue-200 font-black text-blue-700 focus:border-blue-500 outline-none transition-all bg-white text-xl" />
                    </div>
                    <div className="space-y-1">
                      <label className="text-[10px] font-black text-blue-600 uppercase tracking-widest ml-1">Stock Actual</label>
                      <input type="number" required value={formData.stock} onChange={e => setFormData({...formData, stock: e.target.value})} className="w-full px-5 py-4 rounded-2xl border-2 border-blue-200 font-black text-blue-700 focus:border-blue-500 outline-none transition-all bg-white text-xl" />
                    </div>
                  </div>
                </div>
              </div>
